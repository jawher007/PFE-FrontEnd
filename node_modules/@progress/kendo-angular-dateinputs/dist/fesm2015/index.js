...
const EMPTY_DATA = [[]];
const VERTICAL_VIEW_CELLS_LENGTH = 5;
const HORIZONTAL_VIEW_CELLS_LENGTH = 4;
const VERTICAL_VIEW_ROWS_LENGTH = 2;
const HORIZONTAL_VIEW_ROWS_LENGTH = 3;
const ACTIONS = {
...
let CenturyViewService = class CenturyViewService {
    ...
    data(options) {
        const { cellUID, focusedDate, isActiveView, max, min, selectedDate, selectionRange = EMPTY_SELECTIONRANGE, viewDate, direction } = options;
        if (!viewDate) {
            return EMPTY_DATA;
        }
        const cellsLength = this.rowLength({ direction });
        const rowsLength = direction === 'horizontal' ? HORIZONTAL_VIEW_ROWS_LENGTH : VERTICAL_VIEW_ROWS_LENGTH;
        const cells = range(0, cellsLength);
        ...
        return range(0, rowsLength).map(rowOffset => {
            const baseDate = addDecades(firstDate, rowOffset * cellsLength);
...
const EMPTY_DATA$1 = [[]];
const VERTICAL_VIEW_CELLS_LENGTH$1 = 5;
const HORIZONTAL_VIEW_CELLS_LENGTH$1 = 4;
const VERTICAL_VIEW_ROWS_LENGTH$1 = 2;
const HORIZONTAL_VIEW_ROWS_LENGTH$1 = 3;
const ACTIONS$1 = {
...
let DecadeViewService = class DecadeViewService {
    ...
    data(options) {
        ...
        const cellsLength = this.rowLength({ direction });
        const rowsLength = direction === 'horizontal' ? HORIZONTAL_VIEW_ROWS_LENGTH$1 : VERTICAL_VIEW_ROWS_LENGTH$1;
        const cells = range(0, cellsLength);
        ...
        const today = getToday();
        return range(0, rowsLength).map(rowOffset => {
            const baseDate = addYears(firstDate, rowOffset * cellsLength);
            return cells.map(cellOffset => {
...
const EMPTY_DATA$3 = [[]];
const VERTICAL_VIEW_CELLS_LENGTH$2 = 5;
const HORIZONTAL_VIEW_CELLS_LENGTH$2 = 4;
...
let YearViewService = class YearViewService {
    ...
    data(options) {
        ...
        return range(0, ROWS_LENGTH$1).map(rowOffset => {
            const baseDate = addMonths(firstDate, rowOffset * cellsLength);
            return cells.map(cellOffset => {
...
const scrollable = (children) => div(children, 'k-content k-calendar-content k-scrollable');
...
let MultiViewCalendarComponent = class MultiViewCalendarComponent {
    constructor(bus, element, navigator, renderer, cdr, zone, disabledDatesService) {
        this.bus = bus;
        this.element = element;
        this.navigator = navigator;
        this.renderer = renderer;
